...
"""
    ...
    try:
        ...
        print(f"⚠️ [Summarizer] 로컬 Gemma 모델 응답 실패, Gemini 3.5 Flash 엔진으로 폴백합니다. ({e})")
        try:
...
def generate_briefing(filtered_news, failed_sources=None):
    ...
    news_body = ""
    for i, n in enumerate(filtered_news[:10], 1):
        color_emoji = "🔵"
        if "보안" in n['category'] or "사고" in n['category']: color_emoji = "🔴"
        elif "의료" in n['category'] or "병원" in n['category']: color_emoji = "🟢"
        
        safe_title = n['title'].replace('<', '&lt;').replace('>', '&gt;')
        safe_desc = n['description'].replace('<', '&lt;').replace('>', '&gt;')
        
        news_body += f"{color_emoji} <b>{safe_title}</b>\n"
        news_body += f"   <i>{n['source']}</i> | {safe_desc[:100]}...\n"
        news_body += f"   🔗 <a href='{n['link']}'>기사 읽기</a>\n\n"
    
    ai_content = get_ai_insight(filtered_news)
    ...
    if failed_sources:
        footer += "⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n"
        footer += "⚠️ <b>스크랩 지연/오류 채널 안내</b>\n"
        for issue in failed_sources:
            safe_issue = issue.replace('<', '&lt;').replace('>', '&gt;')
            footer += f"- {safe_issue}\n"
        footer += "<i>해당 채널은 복구 및 대체 주소 반영을 위해 진화 작업을 진행 중입니다.</i>\n\n"
