...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const init = async () => {
        ...
        getEl('run-harness-btn')?.addEventListener('click', async () => {
            getEl('run-harness-btn').textContent = "실행 중...";
            try {
                await fetch('/api/run_harness', { method: 'POST' });
                alert("AI 딥 스크래핑 및 브리핑 요약이 시작되었습니다.\n텔레그램 메신저를 확인해 주세요.");
            } catch(e) {
                alert("실행 실패");
            } finally {
                getEl('run-harness-btn').textContent = "🚀 브리핑 실행";
            }
        });
    ...
    window.removeSourceItem = async (idx) => { 
        if (confirm("채널 삭제?")) { 
            const source = newsSources[idx];
            if (source.id) {
